  ...
  return items
    .map((item) => {
      ...
      const metaTag = isExternal
        ? '<span class="ds-tag resource-tag resource-tag-external" data-color="accent">External</span>'
        : `<span class="ds-tag resource-tag" data-variant="outline">${item.itemCount} items</span>`;
      const authorTag =
        isExternal && item.author?.name
          ? `<span class="ds-tag resource-tag" data-variant="outline">by ${escapeHtml(item.author.name)}</span>`
          : '';
      ...
      return `
        <article class="ds-card resource-item${isExternal ? ' resource-item-external' : ''}" data-color="neutral" data-path="${escapeHtml(item.path)}" role="listitem">
          <button type="button" class="resource-preview">
            <div class="resource-info">
              ...
              <div class="resource-meta">
                ${metaTag}
                ${authorTag}
                ${item.tags?.slice(0, 4).map((tag) => `<span class="ds-tag resource-tag" data-variant="outline">${escapeHtml(tag)}</span>`).join('') || ''}
                ${item.tags && item.tags.length > 4 ? `<span class="ds-tag resource-tag" data-variant="outline">+${item.tags.length - 4} more</span>` : ''}
              </div>
            </div>
          </button>
          <div class="resource-actions">
            <a href="${githubHref}" class="ds-button" data-variant="secondary" data-size="sm" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" title="${isExternal ? 'View repository' : 'View on GitHub'}">${isExternal ? 'Repository' : 'GitHub'}</a>
          </div>
        </article>
      `;
    })
