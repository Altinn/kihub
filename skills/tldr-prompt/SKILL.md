...
considered **Ambiguous Queries**
  - If the user provides raw text without a **specific file** or **URL**, identify the topic:
    * Prompts, agents, instructions, collections → Search workspace first
      - If no relevant files found, check https://github.com/Altinn/Kitt_KI_Hub and resolve to
      https://raw.githubusercontent.com/Altinn/Kitt_KI_Hub/refs/heads/main/{{folder}}/{{filename}}
      (e.g., https://raw.githubusercontent.com/Altinn/Kitt_KI_Hub/refs/heads/main/agents/example.agent.md)
...
1. **Identify topic category**:
   - Workspace files → Search ${workspaceFolder} for .agent.md, .instructions.md, .skills.md
     - If NO relevant files found, or data in files from `agents`, `instructions`, or `skills` folders is irrelevant to query → Search https://github.com/Altinn/Kitt_KI_Hub
...
2. **Search strategy**:
   ...
   - For GitHub Altinn/Kitt_KI_Hub: Fetch raw content from https://raw.githubusercontent.com/Altinn/Kitt_KI_Hub/refs/heads/main/
   - For documentation: Use fetch tool with the most relevant URL from above

3. **Fetch content**:
   - Workspace files: Read using file tools
   - GitHub Altinn/Kitt_KI_Hub files: Fetch using raw.githubusercontent.com URLs
