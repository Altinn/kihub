...
- Q: How are Norwegian and English handled? → A: Language-agnostic tokenization (PostgreSQL `simple` text-search configuration) that matches the literal terms present in either Norwegian or English metadata. True cross-lingual matching (a Norwegian query finding English-only content by meaning) is NOT provided this phase — that is a semantic capability deferred with the later semantic phase.
...
- **Different phrasing / word forms**: A query using a different word form than the metadata may or may not match depending on tokenization; exact stemming across languages is not guaranteed this phase (see the language clarification). Matching is over the literal terms present.
- **Query in the other language**: A Norwegian query matches Norwegian terms in the metadata and an English query matches English terms; a Norwegian query does **not** find English-only content by meaning (cross-lingual/semantic matching is deferred).
...
- **FR-018**: Norwegian and English queries MUST match the literal terms present in the respective-language metadata (language-agnostic tokenization); cross-lingual/semantic matching is out of scope this phase.
...
- **Language handling**: A language-agnostic tokenization (PostgreSQL `simple` configuration) is assumed so both Norwegian and English terms match literally without wrong-language stemming; a language-specific configuration can be layered later. True cross-lingual/semantic matching is deferred with the semantic phase.
